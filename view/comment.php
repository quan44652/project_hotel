...
if (isset($user_id)) {
    $sql = "SELECT `booking_detail`.`status` as `status`,`booking`.`user_id` as `user_id` from booking_detail ,booking where `booking_detail`.`booking_id` = `booking`.`booking_id` and `user_id` = '{$user_id}'";
    ...
    <section class="comment section-m2">
        ...
        ?>
            ...
            <?php
                ...
                echo '<h1>Bạn cần đặt phòng mới có thể gửi phản hồi</h1>';
            }
        }
        else {
